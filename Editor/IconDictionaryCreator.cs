...
namespace UnityFolderIcon.Editor
{
    public class IconDictionaryCreator : AssetPostprocessor
    {
        ...
        private static bool ContainsIconAsset(string[] assets)
        {
            ...
            {
                if (ReplaceSeparatorChar(Path.GetDirectoryName(str)) == AssetsPath)
                {
                    return true;
                }
            }
        ...
        internal static void BuildDictionary()
        {
            ...
            {
                var texture = (Texture)AssetDatabase.LoadAssetAtPath($"{AssetsPath}/{f.Name}", typeof(Texture2D));
            ...
            {
                var folderIconSO = (FolderIconSO)AssetDatabase.LoadAssetAtPath($"{AssetsPath}/{f.Name}", typeof(FolderIconSO));
